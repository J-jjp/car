...
public:
    ...
    {
        if (counterShield < 1350)// 确保在环岛检测被触发后的一段时间内不会再次触发
        {
            counterShield++;
            return false;
        }
        ...
        //从下往上搜索
        for (int i = 3; i < track.widthBlock.size(); ++i)
        {   
            //std::cout<<"\tle:"<<track.stdevLeft<<"\tri:"<<track.stdevRight<<std::endl;
            //上行宽度大于下行，并且宽度大于图像列宽0.6并且行大于30，左点集标准差大于120，右点集标准差小于50，判断赛道平直程度
            if (track.widthBlock[i].y - track.widthBlock[i-2].y>20&&((track.stdevLeft>75 &&track.stdevRight<45)||(track.stdevRight>75&&track.stdevLeft<45))
            &&(ringStep == RingStep::None||ringStep == RingStep::Entering)) // 搜索突然变宽的路径行数
            {
                    ...
                    for(int ii = withi1+20; ii < track.widthBlock.size()-10; ii++){        //从找到角点下往上搜索最小点
        ...
        if(ringStep == RingStep::Entering||ringStep == RingStep::Inside){
           bool phase=false;
            ...
            for (int i = track.widthBlock.size(); i >1; i--){
                if(track.widthBlock[i].y-track.widthBlock[i+1].y>30&&track.widthBlock[i].y>COLSIMAGE*0.65&&
                track.widthBlock[i+1].y<COLSIMAGE*0.5&&track.widthBlock[i].x<150&&track.widthBlock[i].x>50&&rightpoint==false&&rightpoint2==false&&phase==false){
        ...
        if(ringStep == RingStep::Exiting){
            if((ringType == RingType::RingRight&&track.stdevLeft<85&&track.pointsEdgeLeft.back().x<ROWSIMAGE/4)||
            (ringType == RingType::RingLeft&&track.stdevRight<85&&track.pointsEdgeRight.back().x<ROWSIMAGE/4))
               finish++;
            std::cout<<"没出";
            if(finish>3){
